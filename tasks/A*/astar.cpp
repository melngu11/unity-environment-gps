...
void aStar(int sx, int sy, int gx, int gy, vector<vector<double>>& dists, vector<vector<pair<int, int>>>& prevs, vector<vector<char>>& grid) {
        ...
        for (int dx = -1; dx <= 1; dx++) { //checks for constraint of grid, coordinates must be postiive
            for (int dy = -1; dy <= 1; dy++) {
                ...
                if (is_valid(nx, ny, grid)) {
                    double new_cost = cost + compute_distance(x, y, nx, ny) + compute_distance(nx, ny, gx, gy);
...
int main() {
    ...
    aStar(0, 0, N - 1, N - 1, dists, prevs, grid);
